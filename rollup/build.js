...
function concatenate_files() {
	...
	const files_to_concat = Object.keys(get_build_json('navdeepghai'))
		.filter(filename => filename.startsWith('concat:'));

	files_to_concat.forEach(output_file => {
		const input_files = get_build_json('navdeepghai')[output_file];

		const file_content = input_files.map(file_name => {
			let prefix = get_app_path('navdeepghai');
			let is_frappe = file_name.includes("frappe") || file_name.includes("lib/")
			if(is_frappe){
				prefix = get_app_path('frappe');
			}
			...
			if (file_name.startsWith('node_modules/')) {
				if(is_frappe){
					...
					prefix = path.resolve(get_app_path('navdeepghai'), '..');
				}
			}
